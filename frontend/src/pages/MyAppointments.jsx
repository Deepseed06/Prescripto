  ...
  const initPay = (order) => {
    const options = {
      ...
      handler: async (response) => {
        ...
        try{
          const {data} = await axios.post(backendUrl + '/api/user/payment-verify', {response}, {headers:{token}})
          if(data.success){
           fetchAppointments()
  ...
  const appointmentRazorpay = async(appointmentId) => {
    try {
      ...
      if(!data.success){
        ...
        // initPay(data.order)
        return toast.error('could not initiate payment')
        ...
        appointments.slice(0,3).map((item, index) =>(
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
           ...
           <div className='flex-1 text-sm text-zinc-600 '>
            ...
            <p className='text-xs mt-1'>Date & Time: <span className='text-sm text-neutral-600 font-medium'>{slotDateFormat(item.slotDate)} | {item.slotTime}</span></p>
           </div>
           <div></div>
           <div className='flex flex-col gap-2 justify-end'>
            {
              !item.cancelled && !item.payment && !item.isCompleted &&
              <button className='sm:min-w-48 py-2 border hidden rounded text-stone-500 bg-indigo-100 '>Paid</button> 
            }
            {
              !item.cancelled && !item.payment && !item.isCompleted ? (
                <>
